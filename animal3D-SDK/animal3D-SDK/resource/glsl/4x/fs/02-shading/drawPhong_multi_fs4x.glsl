...
float specular(vec4 _viewDirection, vec4 _reflectionDirection, float _specularStrength = 0.7, int _shininess = 3);
...
void main()
{
	...
	rtFragColor = texture(uTex_dm, vec2(vTexcoord)) * (finalAmbient + finalSpecular + finalDiffuse);
}
