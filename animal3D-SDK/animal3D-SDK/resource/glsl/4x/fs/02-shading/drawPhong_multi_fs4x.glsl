...
float specular(vec4 _viewDirection, vec4 _reflectionDirection, float _specularStrength, int _shininess);
...
void main()
{
	...
	for (int count = 0; count < uLightCt; count++)
	{
		...
		finalSpecular += specular(viewDirection, reflectionDirection, 1.0, 4) * uLightCol[count];
